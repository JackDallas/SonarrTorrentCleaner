...
func SonarrCheckerLoop(running *bool, config *Config, db *bolt.DB) {
	...
	for *running {
		...
		if err == nil {
			...
			for _, currentQueueItem := range queue {
				//Ignore Queued Items if configured
				if config.IgnoreQueued && currentQueueItem.Status == "Queued" {
					continue
				}
				// Check its a torrent
								...
								if currentQueueItem.Sizeleft == 0 {
									...
									if currentQueueItem.Sizeleft > prevQueueItem.Item.Sizeleft {
										...
										dbItem := SonarrQueueItemDBEntry{
											Item:        currentQueueItem,
											LastChecked: time.Now(),
											FirstSeen:   prevQueueItem.FirstSeen,
										}
										data, err := json.Marshal(dbItem)
										if err != nil {
											log.Error("Error marshalling queue item: " + err.Error())
										...
										fmt.Printf("Time since Lastcd .. Progress %f minutes, Timeout is set to %f\n", timeSinceMinutes, config.NoProgressTimeoutMinutes)
